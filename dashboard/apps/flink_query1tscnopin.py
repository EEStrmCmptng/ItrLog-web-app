...
uname='query1tscnopin'
loc='/home/handong/flink/query1tscnopin'
workload_loc='/home/handong/flink/query1tscnopin/combined.csv'
...
'''
def genintlogfig{uname}(i):
    return html.Div([
        dcc.Textarea(
            id='intlogfig{uname}-textarea-'+str(i),
            value=str(i-1),
            style={'width': '10%'},
            #disabled=True,
            readOnly=True,
        ...
        dcc.Dropdown(id='intlogfig{uname}-yaxis-'+str(i),
                     value='rx_bytes',
                     style={'width':'60%'},
                     options=taxis_values,
                     placeholder="Select y-axis value",),        
        dcc.Graph(
            id='intlogfig{uname}-'+str(i),
            style={'display': 'inline-block'}
        ),
    ], style={'display': 'inline-block'})
'''
...
layout = html.Div([
    ...
    #dbc.Row([        
    #    dbc.Col(children=[genintlogfig{uname}(i) for i in range(1,17)])
    #])
])
...
for i in range(1, 3):
    @app.callback(
        Output(f'intlogfig{uname}-{i}', 'figure'),
        [Input(f'intlogfig{uname}-rate-{i}', 'value'),
         Input(f'intlogfig{uname}-itera-{i}', 'value'),
         Input(f'intlogfig{uname}-nmappers-{i}', 'value'),
         Input(f'intlogfig{uname}-yaxis-{i}', 'value'),
         Input(f'intlogfig{uname}-textarea-{i}', 'value')]
    )
...
'''
@app.callback(
    [Output('intlogfig{uname}-1', 'figure'),
     Output('intlogfig{uname}-2', 'figure'),
     Output('intlogfig{uname}-3', 'figure'),
     Output('intlogfig{uname}-4', 'figure'),
     Output('intlogfig{uname}-5', 'figure'),
     Output('intlogfig{uname}-6', 'figure'),
     Output('intlogfig{uname}-7', 'figure'),
     Output('intlogfig{uname}-8', 'figure'),
     Output('intlogfig{uname}-9', 'figure'),
     Output('intlogfig{uname}-10', 'figure'),
     Output('intlogfig{uname}-11', 'figure'),
     Output('intlogfig{uname}-12', 'figure'),
     Output('intlogfig{uname}-13', 'figure'),
     Output('intlogfig{uname}-14', 'figure'),
     Output('intlogfig{uname}-15', 'figure'),
     Output('intlogfig{uname}-16', 'figure')],
    [Input('intlogfig{uname}-rate-1', 'value'),
     Input('intlogfig{uname}-itera-1', 'value'),
     Input('intlogfig{uname}-nmappers-1', 'value'),
     Input('intlogfig{uname}-yaxis-1', 'value'),
     Input('intlogfig{uname}-textarea-1', 'value')]
)
def update_intlogagg(rate, itera, nmappers, yaxis, core):
    figs = []
