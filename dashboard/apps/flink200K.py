...
layout = html.Div([
    ...
    html.Div([
        dcc.Dropdown(id='mcd2-xaxis-selector-1', value='SourcenumRecordsOutPerSecond_avg', style={'width':'60%'}, options=axis_values),
    ...
    html.Div([
        dcc.Dropdown(id='mcd2-xaxis-selector-2', value='SourcenumRecordsOutPerSecond_avg', style={'width':'60%'}, options=axis_values),
        dcc.Dropdown(id='mcd2-yaxis-selector-2', value='instructions', style={'width':'60%'}, options=axis_values),
        dcc.Graph(
    ...
    html.Div([
        dcc.Dropdown(id='mcd2-xaxis-selector-3', value='SourcenumRecordsOutPerSecond_avg', style={'width':'60%'}, options=axis_values),
        dcc.Dropdown(id='mcd2-yaxis-selector-3', value='ref_cycles_in_Seconds', style={'width':'60%'}, options=axis_values),
        dcc.Graph(
    ...
    html.Div([
        dcc.Dropdown(id='mcd2-xaxis-selector-4', value='rxBytes', style={'width':'60%'}, options=axis_values),
        dcc.Dropdown(id='mcd2-yaxis-selector-4', value='rxBytesIntLog', style={'width':'60%'}, options=axis_values),
        dcc.Graph(
            id='mcd2-custom-scatter-4',
            style={'display': 'inline-block'}
        ),
    ], style={'display': 'inline-block'}),
    ...
    html.Div([
        html.P('Y-AXIS: ', style={'display': 'inline-block'}),
        dcc.Dropdown(id='mcd2-xaxis-selector-6',
                     value='timestamp_diff',
                     style={'width':'60%'},
                     options=taxis_values,
                     placeholder="Select y-axis value",),
        html.P('CORE: ', style={'display': 'inline-block'}),
        dcc.Dropdown(id='mcd2-yaxis-selector-6',
                     value=0,
                     style={'width':'60%'},
                     options=[x for x in range(0, 16)],
                     placeholder="Select core"),
        ...
        html.P('ITERATION: ', style={'display': 'inline-block'}),
        dcc.Dropdown(id='mcd2-zaxis-selector-6',
                     value=0,
                     style={'width':'60%'},
                     options=[x for x in range(0, 10)],
                     placeholder="Select a iteration number",),
        html.P('POLICY: ', style={'display': 'inline-block'}),
        dcc.Dropdown(id='mcd2-aaxis-selector-6',
                     value="ondemand",
                     style={'width':'60%'},
                     options=["ondemand", "conservative","performance", "schedutil", "powersave", "userspace"]),
        html.P('ITR: ', style={'display': 'inline-block'}),
        dcc.Dropdown(id='mcd2-baxis-selector-6',
                     value=1,
                     style={'width':'60%'},
                     options=[1, 2, 100, 200, 400, 600]),
        html.P('DVFS: ', style={'display': 'inline-block'}),
        dcc.Dropdown(id='mcd2-caxis-selector-6',
                     value="1",
                     style={'width':'60%'},
                     options=["1", "0c00", "0e00", "1000", "1200", "1400", "1600", "1800", "1a00"]),
        dcc.Graph(
            id='mcd2-custom-scatter-6',
            style={'display': 'inline-block'}
        ),
    ], style={'display': 'inline-block'}),
...
for i in range(5, 7):
    @app.callback(
        ...
        [Input('mcd2-xaxis-selector-'+str(i), 'value'),
         Input('mcd2-yaxis-selector-'+str(i), 'value'),
         Input('mcd2-zaxis-selector-'+str(i), 'value'),
         Input('mcd2-aaxis-selector-'+str(i), 'value'),
         Input('mcd2-baxis-selector-'+str(i), 'value'),
         Input('mcd2-caxis-selector-'+str(i), 'value')]
    )
    def update_custom_scatter2(xcol, ycol, zcol, acol, bcol, ccol):
        df = pd.DataFrame()
        fname=f"/home/handong/flink/10_12_2023_5.15.89_itrlog/query1_cores16_frate200000_600000_fbuff-1_itr{bcol}_{acol}dvfs{ccol}_repeat{zcol}/ITRlogs/linux.flink.dmesg._{ycol}_{zcol}"
